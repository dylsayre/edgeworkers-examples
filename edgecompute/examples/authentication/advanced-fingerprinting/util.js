...
export function getFingerprintRawString(buffer) {
  ...
  */
  ...
  const EXTENSION_SIGNATURE_ALGORITHMS = 13;
  /* 
  * Extension: signature_algorithms = 0x000d = 13
  * - List of signature algorithm pairs client supports
  */
  let signature_algorithms_joined = '';
  if (extensions_parsed[EXTENSION_SIGNATURE_ALGORITHMS]) {
    // Get the signature algorithms data, which starts with a 2-byte length field
    const signature_algorithms_data = extensions_parsed[EXTENSION_SIGNATURE_ALGORITHMS].extension_data;
    
    // Convert the entire extension data to Uint16Array
    const signature_algorithms_array = uint8to16(signature_algorithms_data);
    
    // The first element is the length value, so we slice to skip it
    // No need to filter GREASE values as they don't apply to signature algorithms
    const algorithms = Array.from(signature_algorithms_array.slice(1));
    
    signature_algorithms_joined = algorithms.join('-');
  }
...
export function advancedFingerprint(buffer) {
  const fingerprint_input = getFingerprintRawString(buffer);
  return fingerprint_input

  //removing hash function for testing. We may decide to use a hash, however that would need to be reworked and datapoints would need to be hashed seperatly to handle conditional blocking.
